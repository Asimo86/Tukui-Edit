...
TukuiDB.PostCastStart = function (self, event, unit)
	if self.interrupt then
		...
		self:SetStatusBarColor(0, 1, 1, 0.5)
	end
end
...
local function CheckRoll(self, event, unit)
	...
	if (TukuiDB.myclass == "PALADIN" and UnitAura("player", GetSpellInfo(25780))) and GetCombatRatingBonus(CR_DEFENSE_SKILL) > 100 or
	(TukuiDB.myclass == "WARRIOR" and GetBonusBarOffset() == 2) or
	(TukuiDB.myclass == "DEATHKNIGHT" and UnitAura("player", GetSpellInfo(48263))) or
	...
	if event == "PLAYER_ENTERING_WORLD" then
		if TukuiDB.myclass == "PALADIN" or TukuiDB.myclass == "DEATHKNIGHT" then
			RollUpdater:UnregisterEvent("UPDATE_BONUS_ACTIONBAR")
		elseif TukuiDB.myclass == "DRUID" or TukuiDB.myclass == "WARRIOR" then
			RollUpdater:UnregisterEvent("UNIT_AURA")
		end
