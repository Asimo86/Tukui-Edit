...
TempEnchant1:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-368), TukuiDB.Scale(-22))
		...
		if ( buff.consolidated ) then
			...
			if ( (index > 1) and (mod(index, rowbuffs) == 1) ) then
				if ( index == rowbuffs+1 ) then
					buff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-368), TukuiDB.Scale(-92))
				else
					buff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-368), TukuiDB.Scale(-22))
				end
				aboveBuff = buff;
			elseif ( index == 1 ) then
				local mainhand, _, _, offhand = GetWeaponEnchantInfo()
...
local function UpdateDebuffAnchors(buttonName, index)
	...
	if index == 1 then
		debuff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-368), TukuiDB.Scale(-140))
	else
